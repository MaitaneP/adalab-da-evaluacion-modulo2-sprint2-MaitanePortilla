...
class Universidades_hipolabs_api:

    def extraccion_json(self, pais):
        """Este método realiza la extracción de los datos de la API de "Universities Hipolabs". Realiza la extracción de los 
    ...
    def extraccion_api_df(self, mi_pais, df_origen):
        """Este método realiza la extracción de los datos de la API de "Universities Hipolabs" llamando al método . Realiza la extracción de los datos en formato .json, mostrando tanto el código
        ...
        su número de filas y columnas y finalemnte concatena ese dataframe con el dataframe original introducido como argunmento. Guarda el resultado en .csv y .pkl.
        Parámetros:
            - pais (str): país del que queremos extraer los datos
        ...
        """
        self.mi_pais = mi_pais
        resultado = self.extraccion_json(mi_pais)
        print(f'Nº de elementos de la lista resultado de {mi_pais}: {len(resultado)}')
        print(f'Las keys de cada diccionario de {mi_pais} son {len(resultado[0])}: {resultado[0].keys()}')
        df_pais = pd.DataFrame(resultado)
        print(f'Añadimos al dataframe resultado las {df_pais.shape[0]} filas y {df_pais.shape[1]} columnas de {mi_pais}')
        df_origen = pd.concat([df_origen, df_pais], axis=0, ignore_index=True)
        df_origen.to_csv('../datos/universidades_arg-can_usa_clase.csv')
        df_origen.to_pickle('../datos/universidades_arg-can_usa_clase.pkl')
        return df_origen



    # EDA. ANÁLISIS EXPLORATORIO DE DATOS
    def explorar_df(self, dataframe, nombre = ''):
        """Este método realiza la exploración inicial de un dataframe dado:
    ...
    def limpieza(self, df):
        """Este método realiza la limpieza completa del dataframe con los datos de la API de "Universities Hipolabs" mientras printea mensajes indicando los pasos realizados correctamente:
        ...
        df.to_csv('../datos/universidades_arg-can_usa_limpio_clase.csv')
        df.to_pickle('../datos/universidades_arg-can_usa_limpio_clase.pkl')
        return df
    ...
    def crear_bbdd(self, datos_acceso, nombre_bbdd):
        """Este método crea la base de datos de MySQL Workbench.
    ...
    def crear_insertar_tabla2(self, access_cred, query):
        """Este método crea las tablas e inserta los datos en la base de datos de MySQL Workbench.
                - Printea un mensaje indicando que la conexión se ha realizado con éxito
                - Comprueba si la BBDD existe antes de crearla
                - Printea el cursor después de ejecutarlo como comprobación
                - Muestra un mensaje de error si la conexión falla indicando información de utilidad a cerca del error.
            ...
            Return: None.
        """ 
        self.access_cred = access_cred
        self.query = query
        cnx = mysql.connector.connect(**access_cred)
        mycursor = cnx.cursor()
        try: 
